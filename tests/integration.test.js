...
console.log("\n[1] The golden path, end to end");
{
  ...
  ok("QR carries the ticket link", global.__QR === "https://pfa.example/seat-booking.html?ticket=" + r.bookingId, global.__QR);
  ok("viewing needs a session", (await err(fns.viewTicket, { auth: null, data: { bookingId: r.bookingId } }))?.code === "unauthenticated");
  const view = await fns.viewTicket(asUser("door-scan", { bookingId: r.bookingId }));
  ok("anyone with the link can view the paid ticket", view.status === "paid" && (view.seats || []).join() === "A1,A2", view);
  ok("the ticket view carries nothing personal", !JSON.stringify(view).includes("k@example.com"), view);
  const blank = await fns.viewTicket(asUser("door-scan", { bookingId: "not-a-real-booking" }));
  ok("an unknown code shows nothing", blank.status === "none", blank);
